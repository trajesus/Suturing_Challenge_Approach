...
class Task2_Loader:
    def __init__(self, device, df, video_path, batch_size=2, size=(256, 256), num_workers=4,
                 mean=torch.tensor([0.485, 0.456, 0.406, 0.5]), std=torch.tensor([0.229, 0.224, 0.225, 0.5])):
        self._video_path = video_path
        self._df = df
        self._df_has_labels = df.columns.str.contains('OSATS_RESPECT').any()
        self._size = size
    ...
    def _prepare_data_loaders(self, task=2): 
        dataset = VideoDataset(self._df, self._cfg, self._video_path, self._transformer, self._mean, self._std)
        
        dataset.set_task_two(has_label=all(col in self._df.columns for col in ['OSATS_RESPECT', 'OSATS_MOTION', 'OSATS_INSTRUMENT', 'OSATS_SUTURE', 'OSATS_FLOW', 'OSATS_KNOWLEDGE', 'OSATS_PERFORMANCE', 
                                         'OSATS_FINAL_QUALITY']))
        loader = DataLoader(
            dataset,
    ...
    def get_loader(self):
        return self._prepare_data_loaders(task=2)
